...
def _verify_front(result):
    """The same convergence check, for a Pareto result.

    ``.achieved`` reports the best of each metric across the front, so those are
    the numbers a reader acts on and those are the designs worth re-checking.
    Verifying only the first front point would miss exactly the case that
    matters: a front whose headline figure comes from an under-resolved corner
    of design space.
    """
    import warnings
    ...
    for k, t in enumerate(result.targets):
        ...
        except Exception:                            # never fail a finished run
            return
