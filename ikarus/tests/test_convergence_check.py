...
def _front(atom, params_list, n_orders):
    ...
    Inventing F by hand makes the check compare a made-up number against a
    computed one, which warns for the wrong reason."""
    ...
    result.F = np.array([_objective_of(result, p, n_orders) for p in params_list],
                        dtype=float)
    return result
...
def test_pareto_front_is_convergence_checked():
    """The front's headline designs must be re-checked at a higher truncation."""
...
def test_pareto_checks_every_metrics_champion():
    """`.achieved` reports the best of EACH metric, so each champion is checked
    -- not merely the first point on the front."""
    from ikarus.inverse.optimize import _verify_convergence
...
def test_pareto_check_never_breaks_a_finished_run():
    """Verification is a courtesy on a completed optimisation; if re-evaluating
    a front point fails, the result must still come back."""
    from ikarus.inverse.optimize import _verify_convergence
